 ...
 * healing vials, refreshing flasks, broadswords and great knives. The traveller can also speak to
 * the player
 *
 * @see Merchant
 * @see Speakable
...
public class Traveller extends Merchant implements Speakable {
   ...
   * their own selling price. A speak action is also return.
   */
  @Override
  public ActionList allowableActions(Actor otherActor, String direction, GameMap map) {
    ...
    if (otherActor.hasCapability(Status.HOSTILE_TO_ENEMY)) {
      if (random.nextInt(100) < 25) {
        actions.add(
            new BuyAction(new HealingVial(), Math.round(BASE_HEALING_VIAL_BUY_PRICE * 1.5f)));
      } else {
      ...
      if (random.nextInt(100) < 10) {
        actions.add(
            new BuyAction(new RefreshingFlask(),
                Math.round(BASE_REFRESHING_FLASK_BUY_PRICE * 0.8f)));
      } else {
  ...
  /**
   * Returns a monologue that is said by the traveller.
   *
  ...
  public String speak(Actor listener) {
    ...
    if (listener.hasCapability(Ability.GREAT_SLAM)) {
      monologues.add(
          "Ooh, that’s a fascinating weapon you got there. I will pay a good price for it. You wouldn't get this price from any other guy.");
    }

    if (!listener.hasCapability(Status.ABXERVYER_KILLER)) {
      monologues.add(
          "You know the rules of this world, and so do I. Each area is ruled by a lord. Defeat the lord of this area, Abxervyer, and you may proceed to the next area.");
    }

    if (listener.hasCapability(Ability.GREAT_SLAM) && listener.hasCapability(
        Status.ABXERVYER_KILLER)) {
      monologues.add(
          "Congratulations on defeating the lord of this area. I noticed you still hold on to that hammer. Why don’t you sell it to me? We've known each other for so long. I can tell you probably don’t need that weapon any longer.");
    }
